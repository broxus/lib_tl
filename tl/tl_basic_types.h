...
template <typename T, typename = decltype(std::declval<T>().write(std::declval<details::LengthCounter &>()))>
...
class flags_type {
 public:
  Flags v = 0;
  static_assert(sizeof(Flags) == sizeof(int32), "flags_type are allowed only wrapping int32 flag types!");
 ...
 public:
  ...
  void write(Accumulator &to) const {
    ...
    if (size == 0) {
      ...
      Writer<Accumulator>::Put(to, size | (static_cast<uint32>(static_cast<uchar>(v[0])) << 8));
    } else if (size == 2) {
      Writer<Accumulator>::Put(to, size | (static_cast<uint32>(static_cast<uchar>(v[0])) << 8) |
...
inline bytes_type make_bytes(bytes::const_span buffer) {
  return make_bytes(QByteArray(reinterpret_cast<const char *>(buffer.data()), buffer.size()));
}
...
class conditional {
 public:
  ...
  template <typename Inner = details::inner_helper::type<T>, typename = std::enable_if_t<!std::is_same_v<Inner, void>>>
